...
export function WaterResults({ result, jobId, client, embedded=false }: { result: WaterResult; jobId: string; client: WaterClient; embedded?:boolean }) {
  ...
  return <section className="water-results" aria-label="Online water results">
    ...
    {spec && spec.stats.cells===0 && <div className="source-period-note" role="status"><strong>No complete daily coverage for this map period</strong><p>This map requires a valid model value on every selected day at each pixel. Daily results may still be available in the chart and CSV below. Review the daily coverage and choose another period or submit a suitable date range. Missing observations have not been replaced with zero.</p></div>}
    ...
    {layer && limits && !!spec?.stats.cells && <div className="water-legend"><span>{number(limits[0])}</span><i style={{ background: `linear-gradient(to right, ${palettes[layer.spec.palette].join(",")})` }} /><span>{number(limits[1])} {layer.spec.unit === "1" ? "" : layer.spec.unit}</span><small>Transparent = missing or outside the selected area</small></div>}
    {series && <TimeSeries {...series} unit={series.unit === "1" ? "Fraction (0–1)" : series.unit} />}
    <div className="output-actions"><button disabled={busy||!layer||!spec?.stats.cells} onClick={()=>void savePng()}>Map PNG ↓</button>{asset && <button disabled={busy} onClick={() => void save(asset.file)}>Period GeoTIFF ↓</button>}<button disabled={busy} onClick={() => void save("daily-summary.csv")}>Daily CSV ↓</button><button disabled={busy} onClick={() => void save("period-summary.csv")}>Period CSV ↓</button><button disabled={busy} onClick={() => void save("results.zip")}>All results ↓</button><button disabled={busy} onClick={() => void save("run-manifest.json")}>Run details ↓</button></div>
