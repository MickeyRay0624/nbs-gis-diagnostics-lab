...
export type DiagnosticRequest = {mode:"ganjam"|"custom";name:string;modules:ModuleId[];boundary?:VectorCollection;config?:Job;land_cover?:{resolution:30|50|100;edge_width_m:number;include_mangroves:boolean;count_boundary_as_edge:boolean}};
...
export type DiagnosticCapabilities = {analyst:string;worker_online:boolean;sample_ready:boolean;retention_days:number;max_area_km2:number;land_cover_max_area_km2:number;max_climate_requests:number;modules:{id:ModuleId;title:string;custom_enabled:boolean;reason:string|null}[]};
...
export type DiagnosticResult = {schema:"nbs-online-diagnostics/v1";name:string;prepared_at:string;complete:boolean;scope:string;boundary:VectorCollection;modules:OnlineModule[];layers:OnlineLayer[];sources:Source[];assets:WaterAsset[];validation:{status:"passed";checks:number;complete:boolean;completed_modules:ModuleId[]};tables:{module:ModuleId;title:string;file:string;rows:Record<string,string|number|null>[]}[]};
