...
import type { DiagnosticCapabilities, DiagnosticRequest, DiagnosticClient, SourceUpload } from './client';
import { readVector } from '../analysis/io';
import { LandInputs, defaultLand } from './LandInputs';
...
export function AnalysisWizard({ganjam,diagnostics,water,client,onSubmit,initialPreset='ganjam'}:{ganjam:VectorCollection|null;diagnostics:DiagnosticCapabilities;water:Capabilities;client:DiagnosticClient;onSubmit:(r:AnalysisRequest)=>Promise<void>;initialPreset?:Preset}){
  ...
  const [land,setLand]=useState<NonNullable<DiagnosticRequest['land_cover']>>(defaultLand);
  const [waterDates,setWaterDates]=useState({start:FAYOUM.start,end:FAYOUM.end});
  const [error,setError]=useState<string|null>(null),[busy,setBusy]=useState(false);
  const [sourceFiles,setSourceFiles]=useState<Record<string,SourceUpload>>({}),[uploading,setUploading]=useState(false);
  ...
  function payload(){
    if(uploading)throw new Error('Wait for the source upload to finish.');
  ...
  return <div className="analysis-builder" ref={top}>
    ...
    <div className="builder-layout">
      <section className="builder-panel" aria-label="Analysis setup">
        <nav className="builder-steps" aria-label="Analysis steps">{['Study area','Diagnostics','Review & run'].map((label,i)=><button key={label} aria-current={step===i?'step':undefined} disabled={busy||uploading} onClick={()=>go(i)}><span>{step>i?'✓':`0${i+1}`}</span><div><small>{['WHERE','WHAT','READY'][i]}</small>{label}</div></button>)}</nav>
        <div className="builder-content">
          {step===0&&<>
            ...
            {preset==='custom'&&<>
              <div className="boundary-switch" role="group" aria-label="Boundary input"><button aria-pressed={boundaryMode==='rectangle'} onClick={()=>setBoundaryMode('rectangle')}>Enter coordinates</button><button aria-pressed={boundaryMode==='upload'} onClick={()=>setBoundaryMode('upload')}>Upload boundary</button></div>
          ...
          {step===2&&<>
            ...
            {modules.some(m=>m==='lulc'||m==='fragmentation')&&<div className="review-land"><strong>Landscape calculation settings</strong><p>{land.source==='uploaded'?land.source_name??'Your uploaded maps':preset==='ganjam'?'GLC-FCS30D':'ESA WorldCover'} · {(land.source==='uploaded'?land.rasters?.map(r=>r.year):land.years??(preset==='ganjam'?[2002,2012,2022]:[2020,2021]))?.join(' / ')} · {land.resolution} m grid</p><p>{land.crosswalk?`${new Set(land.crosswalk.map(r=>r.code)).size} target classes`:'Default class crosswalk'}{modules.includes('fragmentation')?` · forest codes ${(land.forest_codes??(preset==='ganjam'?[2]:[10,95])).join(', ')} · ${land.edge_width_m} m edge`:''}</p><small>{land.protected_upload_id||land.oecm_upload_id?'Protection strata included; outside polygons stays a separate group.':'No protection strata supplied.'}</small></div>}
