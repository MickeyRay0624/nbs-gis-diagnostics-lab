...
| GLC-FCS30D cross-language run | `public/data/glcfcs/validation.json` | 174 numeric comparisons and every forest class pixel across three 6,350,994-cell grids; all three transitions and forest metrics |
...
Browser checks cover actual Ganjam runs, module/layer changes, persistent map context, opacity, numerical outputs and downloads. The application reports all seven modules as prepared, and expert review as pending. The map stays mounted; running analysis does not navigate to another page. A 390 px mobile layout and a production build under the GitHub Pages subpath were also checked. Downloaded GeoTIFF, CSV, JSON and a seven-class PNG were inspected; the diagnostic brief contains all seven modules and their field questions.
